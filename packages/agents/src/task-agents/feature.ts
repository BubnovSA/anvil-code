...
- "ADD OPTION/PROPERTY TO EXISTING TYPE" tasks: (1) add the new member to the existing type via \`add_type_member\` — do NOT change the type's generic parameters (e.g. \`<TRouter, TRequest, TResponse>\` stays as is), do NOT split the type, do NOT replace the parent type with a different base, do NOT add unrelated imports. (2) If the task also describes runtime behavior ("call it when X", "use it for Y"), you ALSO need to wire the new option into the existing function/handler — use \`replace_in_file\` to add the call site at the right place. Both pieces are required; the Reviewer blocks type-only changes when the task asked for behavior, AND blocks runtime changes that alter the type signature beyond the new member.
